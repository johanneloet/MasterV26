...
# Johanne: This file was imported from Marias repository. Changes made for specialization project:
# 1. switch from K-fold to StratifiedKFold for hyperparameter optimization.
# 2. define the new function run_svc_with_feature_eval, which incorporates different feature space scenarios as part of validation.
# 3. as segmentation was switched to repetitions based rather than seconds based, the time window arguments are not really relevant anymore.

# Comment below from Maria, regarding the origins of her file:
...
def run_SVC(
    ...
    if opt == True:
        ...
        model_filename = os.path.join(
            save_base_path, f"SVC_{CV_suffix}_{time_window}.joblib"
        )
...
def run_SVC_with_feature_tuning(
    train_df_per_space,
    test_df_per_space,
    class_names=[],
    CV_suffix="",
    ...
    for space in feature_spaces:
        ...
        if space == "FSR_only":
            y_test_FSR = test_df["label"]
        if space == "FSR_only" and label_mapping is not None:
            y_test_FSR = test_df["label"].map(label_mapping)
        ...
        X_test = test_df.drop(
            columns=["label"]
        )  # test_id column is not added bact to test df, therefore only drop label
        # y_test = y_test["label"]
    ...
    return (
        space_and_performance_summaries,
        y_test_FSR,
    )  # assume y_test is the same for every space. It should be as it is only labels
