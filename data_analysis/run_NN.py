...
def plot_confusion_matrix(y_test, y_test_fit, CV_suffix="", class_names=[]):
    ...
    disp.plot(xticks_rotation="vertical", cmap=plt.cm.plasma, values_format=".2f")
...
def run_NN(
    ...
    if opt == True:
        ...
        model = MLPClassifier(
            random_state=random_state, batch_size=min(200, len(X_train))
        )

        # Use GridSearchCV to find the best hyperparameters
        grid_search = GridSearchCV(
            estimator=model, param_grid=param_grid, cv=StratifiedKFold(3), n_jobs=n_jobs
        )
        ...
        model_filename = os.path.join(
            save_base_path, f"NN_{CV_suffix}_{time_window}.joblib"
        )
    ...
    return test_results, train_results, best_params


def run_NN_with_feature_tuning(
    train_df_per_space,
    test_df_per_space,
    class_names=[],
    CV_suffix="",
    ...
    for space in feature_spaces:
        ...
        "below is originally NN function"
        from sklearn.utils import shuffle
        ...
        assert not np.isnan(X_train_pca).any(), "NaN in X_train"
        assert not np.isinf(X_train_pca).any(), "Inf in X_train"
        ...
        if opt == True:
            ...
            # Print the best parameters
            print(f"Best hyperparameters for feature space {space}: {best_params}")

            #  View cross-validation fold results
            cv_results = grid_search.cv_results_
            print(f"Fold-wise test results {space}:")
            for i in range(3):  # Assuming 3-fold cross-validation
                fold_score = cv_results[f"split{i}_test_score"][grid_search.best_index_]
                print(f"Fold {i+1} test score: {fold_score}")
            ...
            precision_test = precision_score(
                y_test_encoded, y_test_fit, average="macro"
            )
            recall_test = recall_score(y_test_encoded, y_test_fit, average="macro")
            ...
            precision_train = precision_score(
                y_train_encoded, y_train_fit, average="macro"
            )
            recall_train = recall_score(y_train_encoded, y_train_fit, average="macro")
    ...
    return space_and_performance_summaries, y_test_labels
