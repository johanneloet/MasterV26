...
def _region_masks(cols: List[str]) -> Tuple[List[int], List[int], List[int], List[int]]:
    ...
    for i, ch in enumerate(cols):
        if ch not in rc_map:
            raise KeyError(
                f"Column '{ch}' non trovata nel mapping fisico (atteso Fsr.01..Fsr.16)."
            )
        r, c = rc_map[ch]
        info.append((i, r, c))
...
def combine_left_right(rep_L: pd.DataFrame, rep_R: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"total_mean_L", "total_mean_R"}.issubset(df.columns):
        load_asym = (df["total_mean_L"] - df["total_mean_R"]) / (
            df["total_mean_L"] + df["total_mean_R"] + eps
        )
...
def aggregate_per_window(
    per_sample: pd.DataFrame, side: str, start_idx: int, stop_idx: int
):
    """
