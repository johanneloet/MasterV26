...
def ExtractIMU_features_repetitions_based(
    ...
    boundaries = build_boundaries(imu_data, fixed_len=target_num_samples)  # crea
        ...
        if norm_IMU == False:
            if HDR and replace_acc_w_HDR:
                ...
                if len(window_accel_X) < target_num_samples:
                    ...
                    print(
                        f"Dropping window with number of samples {len(window_accel_X)}"
                    )
                    time.sleep(10)
                    continue
                else:
                    ...
                    window_accel_X = downsample_channel(
                        window_accel_X, target_num_samples=target_num_samples
                    )
                    window_accel_Y = downsample_channel(
                        window_accel_Y, target_num_samples=target_num_samples
                    )
                    window_accel_Z = downsample_channel(
                        window_accel_Z, target_num_samples=target_num_samples
                    )

                    window_gyro_X = downsample_channel(
                        window_gyro_X, target_num_samples=target_num_samples
                    )
                    window_gyro_Y = downsample_channel(
                        window_gyro_Y, target_num_samples=target_num_samples
                    )
                    window_gyro_Z = downsample_channel(
                        window_gyro_Z, target_num_samples=target_num_samples
                    )

                    window_mag_X = downsample_channel(
                        window_mag_X, target_num_samples=target_num_samples
                    )
                    window_mag_Y = downsample_channel(
                        window_mag_Y, target_num_samples=target_num_samples
                    )
                    window_mag_Z = downsample_channel(
                        window_mag_Z, target_num_samples=target_num_samples
                    )
                    ...
                    if HDR:
                        window_hdr_X = downsample_channel(
                            window_hdr_X, target_num_samples=target_num_samples
                        )
                        window_hdr_Y = downsample_channel(
                            window_hdr_Y, target_num_samples=target_num_samples
                        )
                        window_hdr_Z = downsample_channel(
                            window_hdr_Z, target_num_samples=target_num_samples
                        )
...
def ExtractIMU_features_window_based(
    ...
    """
    Window-based IMU feature extraction.

    Workflow:
        1. Optionally resample full signal using ReconstructedTime
        2. Build containers from rep_id or label runs
        3. Split into fixed windows (no overlap, centered)
