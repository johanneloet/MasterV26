...
def get_Freq_Domain_features_of_signal(signal, signal_name, Fs):
    ...
    # signal_t = np.transpose(signal) old line
    signal_t = np.transpose(
        np.asarray(signal, dtype=float).ravel()
    )  # did an alteration here, to get welch to run later...

    # Compute PSD via Welch algorithmS
    ...
    features[f"psd_max(Hz)_{suffix}"] = freq[
        np.argmax(psd)
    ]  # have not found this uesd in any of the paper I have reads
    features[f"psd_energy{suffix}"] = sum(
        pow(abs(psd), 2)
    )  # https://stackoverflow.com/questions/34133680/calculate-energy-of-time-domain-data
    features[f"psd_entropy_{suffix}"] = entropy(
        psd
