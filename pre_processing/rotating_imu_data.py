...
def compute_rotation(g_vec):
    ...
    cos_angle = np.dot(
        g_vec, z_world
    )  # dot product of two arrays -- cosine_angle = cos(v) -> v angle between the vectors
...
def plot_z_axis_before_and_after(
    filepath_arm, filepath_back, df_arm_rotated, df_back_rotated
):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(
        df_back["ReconstructedTime"], df_back["Axl.Z"], label="Original Axl.Z (Back)"
    )
    plt.plot(
        df_back["ReconstructedTime"],
        df_back_rotated["Axl.Z"],
        label="Rotated Axl.Z (Back)",
        linestyle="--",
    )
    ...
    plt.plot(
        df_arm["ReconstructedTime"],
        df_arm_rotated["Axl.Z"],
        label="Rotated Axl.Z (Arm)",
        linestyle="--",
    )
